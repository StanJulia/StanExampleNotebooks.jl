...
# ╔═╡ c348d09b-5cc7-47eb-9ba4-148e4167ce04
std(df_sm.theta)
...
begin
	...
	if success(rc_om)
  		map_est, cnames = read_optimize(om)
	end
	map_est
end
...
begin
	θ̂ = mean(map_est["theta"])
	d_map = Normal(θ̂, θ̂ * (1-θ̂))
end

# ╔═╡ a46d3d38-3c56-4b43-a7ee-57323dbb1e3b
begin
	pm = PathfinderModel("bernoulli_pm", bernoulli_model, tmpdir)
	rc_sp = stan_pathfinder(pm; data, num_chains=4, num_psis_draws=4000, num_draws=4000)
	if all(success.(rc_sp))
		dfa = StanSample.convert_a3d(read_pathfinder(pm)..., Val(:dataframes))
	end
end

# ╔═╡ a058cd16-6b85-496e-9cd4-85f6a22114ad
let
	str = read(joinpath(pm.tmpdir, "$(pm.name)_log_1.log"), String)
    findfirst("Path [1]", str)
    str = split(str[findfirst("Path [1]", str)[1]:end], "\n")
end

# ╔═╡ aa891570-ad41-43d2-81c9-dd4303343f71
res = create_pathfinder_profile_df(pm)
...
StanPathfinder = "e8ee4b5e-54b2-4408-8575-c3c89e582a15"
...
manifest_format = "2.0"
project_hash = "5775deb753fc86500b2638a33dfde2df81d4295d"
...
git-tree-sha1 = "c818476044cf63e4e2ff8b6ea55a15f96ad084a0"
...
[[deps.StanPathfinder]]
deps = ["CSV", "DataFrames", "DelimitedFiles", "Distributed", "DocStringExtensions", "NamedTupleTools", "Parameters", "Reexport", "StanBase", "StanIO", "Statistics", "Unicode"]
git-tree-sha1 = "c9b45f45290b2e1c53b9044a71557e5cf52a5a1d"
